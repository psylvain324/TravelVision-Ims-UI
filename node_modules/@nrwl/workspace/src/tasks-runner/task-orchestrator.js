...
class TaskOrchestrator {
    ...
    envForForkedProcess(task, outputPath, forwardOutput, forceColor) {
        ...
        const env = Object.assign(Object.assign(Object.assign({}, envsFromFiles), { FORCE_COLOR: forceColor }), process.env);
        if (outputPath) {
    ...
    getCommandArgs(task) {
        const args = Object.entries(task.overrides || {}).map(([prop, value]) => typeof value === 'boolean'
            ? value
                ? `--${prop}`
                : `--no-${prop}`
            : `--${prop}=${value}`);
