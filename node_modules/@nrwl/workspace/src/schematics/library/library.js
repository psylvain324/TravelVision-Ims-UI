...
function updateTsConfig(options) {
    return (host) => workspace_2.updateJsonInTree('tsconfig.base.json', (json) => {
...
function createFiles(options) {
    return schematics_1.mergeWith(schematics_1.apply(schematics_1.url(`./files/lib`), [
        schematics_1.template(Object.assign(Object.assign(Object.assign({}, options), workspace_3.names(options.name)), { cliCommand: file_utils_1.cliCommand(), tmpl: '', offsetFromRoot: workspace_5.offsetFromRoot(options.projectRoot), hasUnitTestRunner: options.unitTestRunner !== 'none' })),
