...
function trailingCommaNeeded(needed) {
    return needed ? ',' : '';
}
function createInsertChange(path, value, position, commaNeeded) {
    return new workspace_1.InsertChange(path, position, `${trailingCommaNeeded(!commaNeeded)}${value}${trailingCommaNeeded(commaNeeded)}`);
}
...
function addOrUpdateProperty(object, properties, value, path) {
    ...
    if (propertyAssignment) {
        ...
        if (propertyAssignment.initializer.kind ===
            ...
            return [
                createInsertChange(path, value, arrayLiteral.elements.end, arrayLiteral.elements.hasTrailingComma),
            ];
        }
        ...
        return [
            createInsertChange(path, `${JSON.stringify(propertyName)}: ${value}`, object.properties.end, object.properties.hasTrailingComma),
        ];
    }
}
