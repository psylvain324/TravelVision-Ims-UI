...
function update() {
    return schematics_1.chain([
        workspace_1.updatePackagesInPackageJson(path_1.join(__dirname, '../../../', 'migrations.json'), '10.0.0'),
        workspace_1.formatFiles(),
        updateImports,
        removeObsoletePackages,
        (_, context) => {
            const logger = logger_1.getLogger();
            logger.info('NX We upgraded Nest.js to v7. Make sure to read the migration guide: https://docs.nestjs.com/migration-guide');
        },
    ]);
}
...
function updateImports(host) {
    ...
    host.visit((path) => {
        if (ig.ignores(path_1.relative('/', path)) || !/\.ts?$/.test(path)) {
            return;
        }
